...
@php
// 1. Cek relasi secara berlapis (langsung ke prodi ATAU lewat mahasiswa dulu)
$jenjangRaw = auth()->user()->prodi->jenjang 
    ?? (auth()->user()->mahasiswa->prodi->jenjang 
    ?? 'D4'); 

// 2. Gunakan pencarian teks agar fleksibel jika di database tertulis D-3, D III, atau Diploma 3
if (str_contains($jenjangRaw, '3') || stripos($jenjangRaw, 'III') !== false) {
    $jenjang = 'D3';
    $targetSks = 108;
    $maxSemester = 6;
} else {
...
// --- Sisa kode hitungan SKS dan IPK ke bawah tetap sama ---
$totalSksPasti = 0;
...
$persenSksBar = ($totalSksPasti / $targetSks) * 100;
if($persenSksBar > 100){
    $persenSksBar = 100;
}

$ipkLabel = 'Belum Ada Nilai';
$ipkClass = 'text-gray-500 bg-gray-100 border-gray-200';

if ($ipk > 0) {
    if ($ipk >= 3.5) {
        $ipkLabel = 'Sangat Baik';
        $ipkClass = 'text-emerald-600 bg-emerald-50 border-emerald-100';
    } elseif ($ipk >= 3.0) {
        $ipkLabel = 'Baik';
        $ipkClass = 'text-blue-600 bg-blue-50 border-blue-100';
    } elseif ($ipk >= 2.0) {
        $ipkLabel = 'Cukup';
        $ipkClass = 'text-amber-600 bg-amber-50 border-amber-100';
    } else {
        $ipkLabel = 'Kurang';
        $ipkClass = 'text-red-600 bg-red-50 border-red-100';
    }
}
@endphp
...
<div class="khs-wrap p-4 md:p-6 bg-slate-50/50 min-h-screen">
    ...
    <div class="bg-[#4f547d] rounded-3xl p-6 mb-6 shadow-lg">
        ...
        <div class="flex items-center gap-2.5 mb-5">
            ...
            <h2 class="text-white text-base font-bold tracking-tight">Ringkasan Akademik (Jenjang {{ $jenjang }})</h2>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-200">
                <div class="flex items-start justify-between mb-2">
                    ...
                    <span class="text-[10px] font-bold border px-2 py-0.5 rounded-md transition-colors {{ $ipkClass }}">
                        {{ $ipkLabel }}
                    </span>
                </div>
            ...
            <div class="bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-200">
                ...
                {{-- 💡 TARGET SKS DINAMIS SEKARANG --}}
                <p class="text-[10px] text-slate-400 mt-1.5">Dari target kelulusan ~{{ $targetSks }} SKS</p>
            </div>
            ...
            <div class="bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-200">
                ...
                <div class="mt-3.5 flex gap-1.5">
                    {{-- 💡 JUMLAH KOTAK PROGRESS SEMESTER MENGIKUTI MAKSIMAL SEMESTER JENJANG --}}
                    @for ($i = 1; $i <= $maxSemester; $i++) 
                        <div class="h-1.5 flex-1 rounded-full {{ $i <= $semesterSaatIni ? 'bg-violet-500' : 'bg-slate-200' }}"></div>
                    @endfor
                </div>
                {{-- 💡 TEKS JUMLAH SEMESTER MAKSIMAL DINAMIS --}}
                <p class="text-[10px] text-slate-400 mt-2">Berjalan di semester {{ $semesterSaatIni }} dari {{ $maxSemester }}</p>
            </div>
        ...
        <div class="bg-[#4f547d] rounded-3xl p-6 lg:col-span-2 shadow-lg flex flex-col justify-between">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 h-full">
                ...
                <div
                    ...
                    <div>
                        <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">IPS Semester Ini</p>
                ...
                <div
                    ...
                    <p class="text-[10px] text-slate-400 mt-4">Dari batas maksimum 24 SKS </p>
                </div>
            ...
            <div class="w-full overflow-x-auto rounded-2xl border border-gray-100 shadow-sm shadow-slate-900/20 bg-white">
                ...
                <table class="w-full min-w-[700px] border-collapse">
                    ...
                    <tbody>
                        ...
                        @foreach($krs as $dataKrs)
                            @foreach($dataKrs->detail as $item)
                                @if(isset($item->pengajar->semester) && $item->pengajar->semester == $semesterDipilih)
                                    ...
                                    {{-- Cek apakah status KRS sudah ACC / Disetujui Dosen Wali --}}
                                    @if(strtolower($item->status_wali) == 'disetujui')
                                        ...
                                        <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors text-gray-800 text-xs md:text-sm">
                                            ...
                                            <td class="px-6 py-3 text-left break-words text-gray-600">
                                                @if(isset($item->pengajar->dosen) && $item->pengajar->dosen->nama_dosen)
                                                    {{ $item->pengajar->dosen->nama_dosen }}
                                                @elseif(isset($item->pengajar->dosen) && $item->pengajar->dosen->nama)
                        ...
                        @if(!$adaData)
                        <tr>
                            <td colspan="6" class="text-center py-10 bg-gray-50">
                                @if($menungguAcc)
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        ...
                                        <span class="text-amber-600 font-bold text-sm">KRS Belum Disetujui Dosen Wali</span>
                                        <span class="text-gray-500 text-xs">Daftar nilai untuk semester ini akan muncul setelah dosen wali menyetujui (ACC) KRS Anda.</span>
                                    </div>
                                @else
                                    <span class="text-xs text-gray-400 font-medium">Belum ada data nilai mata kuliah untuk semester {{ $semesterDipilih }}.</span>
                                @endif
                            </td>
                        </tr>
                        @endif
