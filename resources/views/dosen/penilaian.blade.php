...
<div class="p-6">
    ...
    {{-- ===== STRUKTUR FORM FILTER UTAMA (SUDAH DI-UPDATE) ===== --}}
    <div class="bg-[#4a4f73] p-6 rounded-xl mb-6 text-white" data-aos="fade-up" data-aos-delay="100">
        ...
        $currentKode = $row->mataKuliah->kode_mk ?? $row->kode_mk;
        if($currentKode == request('kode_mk')) {
            ...
            <div class="grid grid-cols-2 xl:grid-cols-6 gap-4 items-end bg-[#3b3a5e] p-4 rounded-xl text-white">
                ...
                <div class="flex flex-col gap-1 col-span-2 xl:col-span-1">
                    ...
                    <select name="id_prodi"
                        class="w-full px-2 py-1.5 text-xs rounded bg-white text-black border border-gray-300 focus:outline-none font-medium">
                ...
                {{-- KELAS (💡 SEKARANG SUDAH DI-REMEMBER) --}}
                <div class="flex flex-col gap-1">
                    ...
                    <select name="id_kelas"
                        ...
                        {{-- Ditambahkan request check agar pas diklik cari pilihan 'A' tidak hilang --}}
                        <option value="{{ $k->nama_kelas }}"
                            {{ request('id_kelas') == $k->nama_kelas ? 'selected' : '' }}>
                            {{ $k->nama_kelas }}
                        </option>
                        @endforeach
                    </select>
                ...
                {{-- DROPDOWN MATA KULIAH (💡 VALUE DIUBAH KE KODE_MK AGAR SINKRON DENGAN UNIQUE) --}}
                <div class="flex flex-col gap-1 col-span-2 xl:col-span-1">
                    <label class="text-xs text-slate-300 font-medium">Mata Kuliah</label>
                    <select name="kode_mk" id="id_pengajar"
                        class="w-full px-2 py-1.5 text-xs rounded bg-white text-black border border-gray-300 focus:outline-none font-medium cursor-pointer">
                        <option value="">Semua Mata Kuliah</option>
                        @foreach(($matkulDiampu ?? collect())->unique('kode_mk') as $row)
                        @php $targetKode = $row->mataKuliah->kode_mk ?? $row->kode_mk; @endphp
                        <option value="{{ $targetKode }}" {{ request('kode_mk') == $targetKode ? 'selected' : '' }}>
                ...
                {{-- TOMBOL CARI (💡 SEKARANG PANJANG SAMPAI UJUNG) --}}
                <div class="col-span-2 xl:col-span-6 mt-2">
                    <button type="submit"
                        class="w-full bg-[#f4a261] hover:bg-[#e76f51] text-white font-semibold text-xs px-4 py-2 rounded shadow transition-all duration-200 h-[34px]">
                        Cari
                    </button>
                </div>
    ...
    <div class="bg-[#4a4f73] p-6 rounded-xl shadow-lg" data-aos="fade-up" data-aos-delay="200">
        ...
        <div class="flex flex-row justify-end items-center mb-4">
            <div class="text-right">
                <span class="text-xs text-slate-300">Total Baris Nilai Mahasiswa: <b
                        class="text-yellow-400 font-mono text-sm">{{ count($mahasiswa) }}</b></span>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-100 text-slate-700 border-b border-slate-200">
                    <tr>
                        ...
                        if(request('kode_mk')) {
                        // Cari data matkul diampu yang kode_mk nya cocok dengan request
                        $pengajar = $matkulDiampu->first(function($row) {
                        return ($row->mataKuliah->kode_mk ?? $row->kode_mk) == request('kode_mk');
                        });
                        ...
                        <th class="text-center px-3 py-3.5 font-semibold">Partisipatif
                            <br>
                            <span
                                class="text-xs font-normal text-slate-500">({{ $mk->persen_partisipatif ?? 0 }}%)</span>
                        </th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors duration-150">
                        ...
                        <td class="px-5 py-3.5 font-semibold text-slate-600 bg-blue-50/30 font-sans text-sm">
                            {{ $m->nama_mk }}
                        </td>
                        ...
                        <td class="px-2 py-3 text-center">
                            <input type="number" name="partisipatif[{{ $id_detail }}]"
                                value="{{ $khsData->partisipatif ?? 0 }}" min="0" max="100" form="formSimpanNilai"
                        ...
                        <td class="px-2 py-3 text-center">
                            <input type="number" name="tugas[{{ $id_detail }}]" value="{{ $khsData->tugas ?? 0 }}"
                                min="0" max="100" form="formSimpanNilai"
                                class="w-14 h-9 bg-slate-50 border border-slate-200 rounded-lg text-center text-slate-800 font-medium focus:bg-white focus:border-blue-500 focus:outline-none transition-all">
                        </td>
                        <td class="px-2 py-3 text-center">
                        ...
                        <td class="px-2 py-3 text-center">
                            <input type="number" name="quiz[{{ $id_detail }}]" value="{{ $khsData->quiz ?? 0 }}" min="0"
                                max="100" form="formSimpanNilai"
                        ...
                        <td class="px-2 py-3 text-center">
                            <input type="number" name="uts[{{ $id_detail }}]" value="{{ $khsData->uts ?? 0 }}" min="0"
                                max="100" form="formSimpanNilai"
                                class="w-14 h-9 bg-slate-50 border border-slate-200 rounded-lg text-center text-slate-800 font-medium focus:bg-white focus:border-blue-500 focus:outline-none transition-all">
                        </td>
                        <td class="px-2 py-3 text-center">
                            <input type="number" name="uas[{{ $id_detail }}]" value="{{ $khsData->uas ?? 0 }}" min="0"
                                max="100" form="formSimpanNilai"
                                class="w-14 h-9 bg-slate-50 border border-slate-200 rounded-lg text-center text-slate-800 font-medium focus:bg-white focus:border-blue-500 focus:outline-none transition-all">
                        </td>
        ...
        <div class="flex justify-between items-center mt-6">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" name="action" value="draft" form="formSimpanNilai"
                    {{ $kpsLocked ? 'disabled' : '' }}
                ...
                <button type="submit" name="action" value="final" form="formSimpanNilai"
                    {{ $kpsLocked ? 'disabled' : '' }}
